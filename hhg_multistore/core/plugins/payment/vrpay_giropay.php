...
class vrpay_giropay extends vrpay_checkout {
	...
	function vrpay_giropay() {
		...
		$this->INFOTEXT		= MODULE_PAYMENT_VRPAY_GIROPAY_INFOTEXT;
		...
		$this->icons = hhg_image(DIR_WS_CATALOG . DIR_WS_ACTUAL_STORE_FILES . DIR_WS_ICONS . 'vrpay/giropay.png') ;
		$this->icons .= (MODULE_PAYMENT_VRPAY_GIROPAY_SHOW_VRPAY == 'True') ? ' ' . hhg_image(DIR_WS_CATALOG . DIR_WS_ACTUAL_STORE_FILES . DIR_WS_ICONS . 'vrpay/vrpay.png') : '';
	}
	...
	function update_status() {
		...
		if (($this->enabled == true) && ((int)MODULE_PAYMENT_PAYPAL_ZONE > 0)) {
			$check_flag = false;
			$check = hhg_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_VRPAY_GIROPAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
			while (!$check->EOF) {
...
//		xtc_db_query("UPDATE ".TABLE_ORDERS." SET orders_status='".$this->order_status."' WHERE orders_id='". xtc_db_input($insert_id)."'");
	...
	function check() {
		if (!isset($this->_check)) {
			$check_query = hhg_db_query("select configuration_value from configuration_" . $this->store_id . "_modules where configuration_key = 'MODULE_PAYMENT_VRPAY_GIROPAY_STATUS'");
	...
	function install() {
		...
		hhg_db_query("insert into configuration_" . $this->store_id . "_modules (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_VRPAY_GIROPAY_STATUS', 'True', '6', '10', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
		hhg_db_query("insert into configuration_" . $this->store_id . "_modules (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_VRPAY_GIROPAY_ZONE', '0', '6', '11', 'xtc_get_zone_class_title', 'xtc_cfg_pull_down_zone_classes(', now())");
		hhg_db_query("insert into configuration_" . $this->store_id . "_modules (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VRPAY_GIROPAY_ALLOWED', '', '6', '12', now())");		
		hhg_db_query("insert into configuration_" . $this->store_id . "_modules (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VRPAY_GIROPAY_SORT_ORDER', '12', '13', '16', now())");
		...
		hhg_db_query("insert into configuration_" . $this->store_id . "_modules (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VRPAY_GIROPAY_VERWENDUNG1', '', '6', '26', now())");
		hhg_db_query("insert into configuration_" . $this->store_id . "_modules (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VRPAY_GIROPAY_VERWENDUNG2', '". STORE_NAME ."', '6', '27', now())");
		hhg_db_query("insert into configuration_" . $this->store_id . "_modules (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VRPAY_GIROPAY_INFOTEXT', '', '6', '28', now())");
		hhg_db_query("insert into configuration_" . $this->store_id . "_modules (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_VRPAY_GIROPAY_SHOW_VRPAY', 'True', '6', '35', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
		
		hhg_db_query("insert into configuration_" . $this->store_id . "_modules ( configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_VRPAY_GIROPAY_ORDER_STATUS_ID', '0',  '6', '50', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
		hhg_db_query("insert into configuration_" . $this->store_id . "_modules ( configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_VRPAY_GIROPAY_ORDER_FAILED_STATUS_ID', '0',  '6', '51', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
		hhg_db_query("insert into configuration_" . $this->store_id . "_modules ( configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_VRPAY_GIROPAY_TMP_STATUS_ID', '0',  '6', '52', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
...
//		}
	...
	function keys($scope = 'all') {
		$keys = array();
		$check_keys = hhg_db_query("select configuration_key from configuration_" . $this->store_id . "_modules where configuration_key LIKE 'MODULE_PAYMENT_VRPAY_GIROPAY_%'" . (($scope == 'all') ? " OR configuration_key LIKE 'MODULE_PAYMENT_VRPAY_SHARED_%' " : "") . "ORDER BY sort_order");
