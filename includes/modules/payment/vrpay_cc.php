...
class vrpay_cc {
	...
	function vrpay_cc() {
		...
		$this->modul_title = MODULE_PAYMENT_VRPAY_CC_TEXT_MODUL_TITLE;
		...
		// logging
		$this->log         = MODULE_PAYMENT_VRPAY_CC_LOG;
		$this->logging     = ((MODULE_PAYMENT_VRPAY_CC_LOGGING == 'True') ? true : false);
		// geozone e.g. (EU)
		$this->zone        = MODULE_PAYMENT_VRPAY_CC_ZONE;
		// checkout function for vrepay
		$this->form_action_url = xtc_href_link("checkout_vrepay_kreditkarte.php", '', 'SSL');

		// iframe layout
		$this->iframeFlag  = ((MODULE_PAYMENT_VRPAY_CC_IFRAMEFLAG == 'True') ? true : false);
		$this->iframe = MODULE_PAYMENT_VRPAY_CC_IFRAME;
	...
	function update_status() {
		...
		if ( ($this->enabled == true) && ((int) $this->zone > 0) ) {
			...
			while ($check = xtc_db_fetch_array($check_query)) {
				...
				if ($check['zone_id'] < 1) {
					$this->epay_log( 'zone_id: ' . $check['zone_id'] . '...keine Landeszone f�r epay Steuerzone + Land hinterlegt --> OK' );
					$check_flag = true;
					break;
				}
	...
	function payment_action() {
		...
		$ft = urlencode(MODULE_PAYMENT_VRPAY_CC_TEXT_ERR01);
		...
		// condition�s
		$this->urlagb = xtc_href_link(MODULE_PAYMENT_VRPAY_CC_URLAGB, '', 'SSL');
		$this->urlagb = substr($this->urlagb, 0, 254);

		// merchantno.
		$this->haendlernr = MODULE_PAYMENT_VRPAY_CC_INSTITUT;
		...
		$this->verwendung2 = substr(MODULE_PAYMENT_VRPAY_CC_VERWENDUNG2, 0, 25);
		...
		$md5 = md5($this->betrag.$this->referenznr.$this->zahlart.MODULE_PAYMENT_VRPAY_CC_ANTWGEHEIMNIS);

		// collect http-postdata
		$post = array(
			'HAENDLERNR'	=> $this->haendlernr,
			'REFERENZNR'	=> substr(MODULE_PAYMENT_VRPAY_CC_ORDERPREFIX, 0, 5) . $this->referenznr,
			'BETRAG'		=> $this->betrag,
		...
		$this->epay_log( "function payment_action() postdata -> " . MODULE_PAYMENT_VRPAY_CC_SERVER . print_r($post, TRUE) );

		// compose HTTP-Query
		$query = http_build_query($post,'','&');

		// HTTP-Basic authorisation
		$auth = MODULE_PAYMENT_VRPAY_CC_INSTITUT . ":" . MODULE_PAYMENT_VRPAY_CC_PASSWORT;

		// cURL init & options
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL,'https://'. MODULE_PAYMENT_VRPAY_CC_SERVER);
		...
		if ($ret == false) {
			...
			$fc = urlencode("CURL");
			$ft = urlencode(MODULE_PAYMENT_VRPAY_CC_TEXT_ERR03 ." [". curl_error($ch) ."]");
			$urlmsg = 'payment_error=' . $this->code . '&FEHLERCODE='. $fc . '&FEHLERTEXT=' . $ft;
		    ...
		    case "302": // redirect = success
				...
				// Sessiondaten mit redirect URL f�llen	zur Weiterverarbeitung
				$_SESSION['vrepaydialog'] = array (
						'redirect'	=> $this->redirect,
//						'message' 	=> MODULE_PAYMENT_VRPAY_CC_TEXT_MSG01,
				);
		    ...
		    default:	// others = systemerror
				...
				// compose error message
				$fc = urlencode("HTTP");
				$ft = urlencode(MODULE_PAYMENT_VRPAY_CC_TEXT_ERR02 . " http-Code [". $info['http_code'] ."]");
				$urlmsg = 'payment_error=' . $this->code . '&FEHLERCODE='. $fc . '&FEHLERTEXT=' . $ft;
	...
	function check() {
		...
		// query for checking module stat --> MODULE_PAYMENT_VRPAY_CC_STATUS
	...
	function install() {
		...
		// vrepay_dialog
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VRPAY_CC_SERVER', 'payinte.vr-epay.de/pbr/transaktion', '6', '2', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VRPAY_CC_INSTITUT', 'vrepay_merchant', '6', '3', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VRPAY_CC_PASSWORT', 'vrepay_password', '6', '4', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VRPAY_CC_ZAHLART', 'KAUFEN', '6', '5', now())");
		...
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VRPAY_CC_ANTWGEHEIMNIS', 'your_secret', '6', '8', now())");
		xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_VRPAY_CC_VERWENDUNG2', 'enjoy xtCommerce shopping', '6', '10', now())");
	...
	function keys() {
		return array(
			'MODULE_PAYMENT_VRPAY_CC_STATUS',
			'MODULE_PAYMENT_VRPAY_CC_ZONE',
			'MODULE_PAYMENT_VRPAY_CC_ALLOWED',
			'MODULE_PAYMENT_VRPAY_CC_SORT_ORDER',
			'MODULE_PAYMENT_VRPAY_CC_SERVER',
			'MODULE_PAYMENT_VRPAY_CC_INSTITUT',
			'MODULE_PAYMENT_VRPAY_CC_PASSWORT',
			'MODULE_PAYMENT_VRPAY_CC_ZAHLART',
			'MODULE_PAYMENT_VRPAY_CC_URLAGB',
			'MODULE_PAYMENT_VRPAY_CC_ORDERPREFIX',
			'MODULE_PAYMENT_VRPAY_CC_ANTWGEHEIMNIS',
			'MODULE_PAYMENT_VRPAY_CC_VERWENDUNG2',
			'MODULE_PAYMENT_VRPAY_CC_IFRAMEFLAG',
			'MODULE_PAYMENT_VRPAY_CC_IFRAME',
			'MODULE_PAYMENT_VRPAY_CC_LOG',
			'MODULE_PAYMENT_VRPAY_CC_LOGGING'
		);
	}
